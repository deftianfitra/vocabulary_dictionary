...
def detail(keyword):
    api_key = '3ff7feb9-e7ec-4f20-9aa3-04be4dd1fc80'
    url = f'https://www.dictionaryapi.com/api/v3/references/collegiate/json/{keyword}?key={api_key}'
    response = requests.get(url)
    definitions = response.json()

    if not definitions:
        return render_template('error.html', message=f'Your word "{keyword}", could not be found')

    if type(definitions[0]) is str:
        return render_template('error.html', message=f'Your word "{keyword}", could not be found', suggestions=definitions)
